...
@Entity
@NamedQueries({
        @NamedQuery(name = Route.SELECT_ALL, query = Route.SELECT_ALL_ROUTES_SQL_QUERY),
        @NamedQuery(name = Route.SELECT_ALL_FOR_RAILWAY_STATION, query = Route.SELECT_RAILWAY_STATION_SQL_QUERY),
        @NamedQuery(name = Route.CUMULATIVE_FREQUENCY_BY_OK_TRAVEL, query = Route.CUMULATIVE_FREQUENCY_SQL_QUERY)
})
...
public class Route {
     ...
     */
    public static final String CUMULATIVE_FREQUENCY_SQL_QUERY = "select sum(r.frequency * 0.75) from Route r join r.operators o where o.name = 'OK Travel'";

    /**
     * The HQL Query the 'select all routes' query.
     */
    public static final String SELECT_ALL_ROUTES_SQL_QUERY = "select r from Route r order by r.number asc";

    /**
     * The HQL Query for the select all routes that go via the Railway Station query.
     */
    public static final String SELECT_RAILWAY_STATION_SQL_QUERY = "select distinct r from Route r where r.startStop.id =  9015 OR r.startStop.id = 9016 OR r.destinationStop.id = 9015 OR r.destinationStop.id = 9016";
